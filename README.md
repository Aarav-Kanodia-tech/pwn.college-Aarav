...
|Command|Usage                              |Syntax             |switches and usage                |
|-------|-----------------------------------|-------------------|----------------------------------|
...
|ls|gives files in directory|ls <dir path>|`-l` gives file type,`-a` show files beginning with .
...
|grep|searches phrases inside file|grep "<word>" <file>|`-e` for multiple searches at once,`-i` for case insensitive search|
|mv|moves file|mv <file> <destination path>|N.A.|
|cp|copies file to another place|cp <file> <destination path>|N.A|
|find|find files that contain a phrase in the name(path)|find <dir in which to search> -name <path>|`-name` for specific words,`-path` for specific path,`not` to remove those matches|
